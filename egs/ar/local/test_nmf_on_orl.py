...
if __name__ == '__main__': 
	...
	if not os.path.exists(nmf_dir):
		os.makedirs(nmf_dir)
	os.system("python local/factorizer.py -in %s -out %s -i %d -n %d -W 0.7 -mW 0.0001 -mH 0.0001" \
			% (data_dir, nmf_dir, num_iter, num_basis))
	print "Basis stored in %s\n" % nmf_dir

	print "Apply NMF (basis constrained to sparseness 0.6)"
	nmf_dir = "%s/nmf_W0.6" % exp_dir
	if not os.path.exists(nmf_dir):
		os.makedirs(nmf_dir)
	os.system("python local/factorizer.py -in %s -out %s -i %d -n %d -W 0.6" \
			% (data_dir, nmf_dir, num_iter, num_basis))
	print "Basis stored in %s\n" % nmf_dir

	print "Apply NMF (basis constrained to sparseness 0.5)"
	nmf_dir = "%s/nmf_W0.5" % exp_dir
	if not os.path.exists(nmf_dir):
		os.makedirs(nmf_dir)
	os.system("python local/factorizer.py -in %s -out %s -i %d -n %d -W 0.5" \
			% (data_dir, nmf_dir, num_iter, num_basis))
	print "Basis stored in %s\n" % nmf_dir
