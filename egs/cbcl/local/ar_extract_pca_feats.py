...
if __name__ == '__main__': 
	...
	if not os.path.isfile(pca_bases_pname):
		sys.exit(1)
	...
	with open(mask_pname, "rb") as f:
		mask = pickle.load(f)
	print "elliptical mask loaded from %s" % mask_pname
	...
	assert(V.shape[0] == np.where(mask==255)[0].size)
