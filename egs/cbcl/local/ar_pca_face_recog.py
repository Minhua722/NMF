...
if __name__ == '__main__': 
	...
	parser.add_argument('--input_dir', '-in', 
			...
			help='data dir with pca feats and labels')
	...
	with open(train_pca_feats_pname, "rb") as f:
		train_pca_feats = pickle.load(f) # each row is pca feats for an image
	...
	for set_name in test_sets:
		...
		with open(test_pca_feats_pname, "rb") as f:
			test_pca_feats = pickle.load(f) # each row is pca feats for an image
		...
		assert(train_pca_feats.shape[0] == test_pca_feats.shape[0])
		D = train_pca_feats.shape[0]
		...
		for t in range(Ntest):
			test_feat = test_pca_feats[:, t].reshape(D, 1)
			ref = test_label[test_count]
			#scores = np.apply_along_axis(np.linalg.norm, 0, train_pca_feats - test_feat)
			#hypo = train_label[np.argmin(scores)]
			# NCC
			hypo =train_label[np.argmax(np.dot(test_feat.T, train_pca_feats))]
